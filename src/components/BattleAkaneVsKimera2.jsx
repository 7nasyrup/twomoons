...
const ATTACK_PATTERNS = [
  ...
  },
  {
    label: '暴走双爪', sequence: [
      ...
      { hits: 1, duration: 200, interval: 0, delayStart: 1000 }
    ]
  },
  {
    label: '時間差双爪', sequence: [
      { hits: 1, duration: 900, interval: 0, delayStart: 0 },
      { hits: 1, duration: 900, interval: 0, delayStart: 900 }
    ]
  },
  {
    label: '疾風三連爪', sequence: [
      { hits: 1, duration: 900, interval: 0, delayStart: 0 },
      { hits: 1, duration: 200, interval: 0, delayStart: 900 },
      { hits: 1, duration: 200, interval: 0, delayStart: 1100 }
    ]
  },
  {
    label: '緩急の三重奏', sequence: [
      { hits: 1, duration: 1000, interval: 0, delayStart: 0 },
...
const getCharInfo = (id) => {
  const map = {
    mutsunori: { name: '睦典', image: '/battle/akane.png', isAlly: true },
    nagisa: { name: '凪砂', image: '/battle/nagisa.png', isAlly: true },
    enemy1: { name: 'キメラ', image: '/character/kimera2.png', isAlly: false },
...
export default function BattleAkaneVsKimera2({ onComplete, playBGM, stopBGM, playSE }) {
  ...
  const [showTutorial, setShowTutorial] = useState(false); // 最初の説明文は消す
                ...
                <div key={enemy.id} className="relative w-full h-[420px] lg:h-[520px] flex flex-col justify-end items-center translate-y-12 lg:translate-y-20">
                  ...
                  <div className="absolute bottom-0 inset-x-0 h-[307px] lg:h-96 flex items-end justify-center">
                    <motion.div
                      ...
                      // 敵の立ち絵のサイズ
                      id={`char-${enemy.id}`}
                      className={`absolute bottom-0 w-[230px] h-[290px] lg:w-80 lg:h-[160px] flex items-end justify-center z-40 ${enemy.isDead ? 'opacity-30 grayscale'
                      ...
                      <AnimatePresence>
                        {enemy.flashTimer > 0 && (
                          <motion.div
                            ...
                            className="absolute inset-x-0 bottom-0 h-full bg-red-600 mix-blend-color z-10 pointer-events-none origin-bottom"
                            style={{
                    ...
                    <AnimatePresence>
                      {showDamageNumbers.filter(d => d.targetId === enemy.id && (d.type === 'damage' || d.type === 'critical')).map(d => (
                        <div
                          key={`fx-wrap-${d.id}`}
                          className="absolute inset-x-0 bottom-0 h-full flex items-center justify-center pointer-events-none z-50 mix-blend-screen scale-[0.8] lg:scale-100"
                          style={{
                            /* 
                             * 【斬撃エフェクトの上下位置調整】
                             * マイナスの数値を大きくすると上に、小さくすると下に移動します。
