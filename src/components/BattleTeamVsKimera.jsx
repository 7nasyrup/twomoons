...
const getCharInfo = (id) => {
  const map = {
    ...
    enemy1: { name: 'キメラα', image: '/character/kimera3.png', isAlly: false },
    enemy2: { name: 'キメラβ', image: '/character/kimera3.png', isAlly: false },
  };
...
export default function BattleTeamVsKimera({ onComplete, playBGM, stopBGM, playSE }) {
  ...
  const [showTutorial, setShowTutorial] = useState(false); // No tutorial state for team fight
  ...
  const executeAllyAttack = useCallback((qteResult) => {
    ...
    if (aliveEnemies.length > 0) {
      ...
      if (playSE) playSE(assetPath('/assets/audio/bgm/+game_sword.mp3'));
    }
  ...
  const advanceTurn = useCallback(() => {
    ...
    // Safeguard: Clear all guarding states and pending timeouts on turn advance
    setGuardingAllies(new Set());
    Object.keys(pendingGuardTimeoutsRef.current).forEach(id => {
      clearTimeout(pendingGuardTimeoutsRef.current[id]);
    });
    ...
    const tick = (timestamp) => {
      ...
      if (phase === 'turn_delay') {
        setTurnTimer(prev => {
          ...
          if (next <= 0) {
            ...
            const isAlly = isAllyTurn(turnId);
            if (isAlly) {
              const ally = allAllies.find(a => a.id === turnId);
              if (!ally || ally.isDead) {
      ...
      if (phase === 'counter_attack' && stateRef.current.counterAttack) {
        ...
        if (ally && enemy && !enemy.isDead) {
          if (playSE) playSE(assetPath('/assets/audio/bgm/+game_counter.mp3'));
          setShakeActive(true);
        ...
        // Safeguard: Clear all guarding states and pending timeouts on counter-attack resolution
        setGuardingAllies(new Set());
        Object.keys(pendingGuardTimeoutsRef.current).forEach(id => {
          clearTimeout(pendingGuardTimeoutsRef.current[id]);
        });
  ...
  return (
    <div className={`absolute inset-0 w-full h-full bg-[#090e17] overflow-hidden select-none z-50 flex flex-col font-orbitron ${shakeActive ? 'animate-battle-shake' : ''}`}>
      ...
      <div className="absolute inset-0 z-0">
        ...
        <img src="/battle/houkai2.png" alt="Background" className="absolute inset-0 w-full h-full object-cover object-[50%_15%] transform-gpu will-change-transform" />
      ...
      <div className="relative flex-1 flex items-stretch px-4 lg:px-12 pt-20 lg:pt-32 pb-16 lg:pb-24 overflow-hidden">
        ...
        {/* ── Allies (Left Column) ── */}
        <div className="w-1/2 flex flex-row-reverse justify-around items-end pb-4 pr-4 translate-x-4 lg:translate-x-8">
          {allies.map(ally => {
                ...
                <div className={`relative flex items-center justify-center ${ally.id === 'nagisa' ? '' : 'top-2 left-2 lg:top-0 lg:left-0'}`}>
                  ...
                  {ally.id === 'mika' && (
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0">
                      <div className="relative w-[125px] h-[187px] lg:w-48 lg:h-64 -translate-y-6 lg:-translate-y-12 -translate-x-32 lg:-translate-x-32">
                        <img
        ...
        <div className="w-1/2 flex flex-col justify-around items-center pl-4 -translate-x-4 lg:-translate-x-8">
          {enemies.map(enemy => {
            ...
            return (
              <div key={enemy.id} className="relative flex flex-col items-center w-full -translate-y-40 lg:-translate-y-72">
                <div className="w-20 lg:w-36 z-50 relative -translate-x-6 lg:translate-x-0 translate-y-[120px] lg:translate-y-[320px]">
                  <div className="flex flex-col items-center">
