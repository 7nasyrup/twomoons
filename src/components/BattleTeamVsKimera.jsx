...
// Turn order
const TURN_ORDER = ['mutsunori', 'nagisa', 'mika', 'enemy1'];
...
// COMPONENT
// ═══════════════════════════════════════════════════════════════════════════════
export default function BattleTeamVsKimera({ onComplete, playBGM, stopBGM, playSE }) {
  // ─── Core State ───
  ...
  const [guardCooldownTrigger, setGuardCooldownTrigger] = useState({ mutsunori: 0, nagisa: 0, mika: 0 });
  ...
  const guardCooldownsRef = useRef({ mutsunori: 0, nagisa: 0, mika: 0 });
  ...
  const isAllyTurn = useCallback((id) => {
    return id === 'mutsunori' || id === 'nagisa' || id === 'mika';
  }, []);
      ...
      if (phase === 'turn_delay') {
        setTurnTimer(prev => {
          ...
          if (next <= 0) {
            ...
            const isAlly = turnId === 'mutsunori' || turnId === 'nagisa' || turnId === 'mika';
            if (isAlly) {
              const ally = allAllies.find(a => a.id === turnId);
              if (!ally || ally.isDead) {
  ...
  return (
    <div className={`absolute inset-0 w-full h-full bg-[#090e17] overflow-hidden select-none z-50 flex flex-col font-orbitron ${shakeActive ? 'animate-battle-shake' : ''}`}>
      ...
      <div className="absolute inset-0 z-0">
        ...
        <img src="/battle/houkai.png" alt="Background" className="absolute inset-0 w-full h-full object-cover scale-[1.2] -translate-y-[15%]" />
      ...
      <div className="relative flex-1 flex items-stretch px-4 md:px-12 pt-32 pb-24 overflow-hidden">
        ...
        <div className="w-1/2 relative pr-4">
          {allies.map(ally => {
            ...
            // 前2人(睦典・凪砂)、後ろ1人(ミカ) のフォーメーション
            // ※ transform を使うとスタッキングコンテキストが生成されてエフェクトの透過が効かなくなるため、ネガティブマージンで中央合わせ(-50%相当)を再現
            const posClass = ally.id === 'mutsunori' ? 'right-[-10%] top-[45%] -mt-[86px] md:-mt-[113px]' :
              ally.id === 'nagisa' ? 'right-[10%] top-[70%] -mt-[102px] md:-mt-[125px]' :
                ally.id === 'mika' ? 'left-[30%] top-[50%] -mt-[105px] md:-mt-[140px]' : '';

            return (
              <div key={ally.id} className={`absolute flex flex-col items-center ${posClass}`}>
                <div className="relative flex items-center justify-center">
                  ...
                  <motion.div
                    ...
                    ${ally.id === 'mutsunori' ? 'w-[130px] h-[173px] md:w-[170px] md:h-[226px] -translate-y-4' : ''}
                    ${ally.id === 'nagisa' ? 'w-[155px] h-[205px] md:w-[200px] md:h-[250px]' : ''}
                    ${ally.id === 'mika' ? 'w-[160px] h-[210px] md:w-[210px] md:h-[280px] -translate-y-4' : ''}
      ...
      <div className="absolute inset-0 z-40 pointer-events-none">
        ...
        <div className="absolute top-4 left-4 md:top-6 md:left-6 pointer-events-auto flex flex-col gap-1.5 md:gap-2">
          {allies.map(ally => {
            // HMR (自動更新) 時に古いStateが残る対策として、常に最新の関数から画像を取得
            const staticInfo = createAllies().find(a => a.id === ally.id) || ally;
            const currentCutin = staticInfo.cutinImage;

            return (
              <div key={ally.id} className="flex items-end gap-2 md:gap-3">
                {/* Portrait */}
                <div className="w-8 h-8 md:w-12 md:h-12 bg-slate-900 border-[1.5px] relative overflow-hidden flex-shrink-0" style={{ borderColor: ally.color, boxShadow: `0 0 10px ${ally.color}40` }}>
                  <img
                    src={currentCutin}
                    alt={ally.name}
                    className={`absolute max-w-none ${currentCutin.includes('/battle/') ? 'w-[150%] object-[center_top]' : 'w-[400%] object-top'}`}
                    style={{ top: currentCutin.includes('/battle/') ? '-10%' : '0%', left: '50%', transform: 'translateX(-48%)' }}
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = ally.image;
                    }}
                  />
                ...
                {/* Status Info */}
                <div className="flex flex-col justify-end w-24 md:w-32 pb-[1px]">
                  <div className="flex justify-between items-end mb-0.5">
                    <span className="font-noto font-bold text-xs md:text-sm tracking-widest drop-shadow-[0_0_5px_rgba(255,255,255,0.8)] leading-none" style={{ color: ally.color }}>
                      {ally.name}
                    </span>
                    <span className="font-orbitron text-[8px] md:text-[9px] text-white tabular-nums leading-none">
                      {ally.hp}<span className="text-white/60 text-[6px] md:text-[7px]">/{ally.maxHp}</span>
                    </span>
                  </div>

                  <div className="flex items-center gap-1.5">
                    <span className="font-orbitron font-bold text-white/80 text-[8px] md:text-[10px] leading-none">HP</span>
                    <div className="flex-1 h-[6px] md:h-[8px] bg-black border p-[1px] relative overflow-hidden" style={{ borderColor: `${ally.color}80` }}>
                      <motion.div
                        className="h-full relative"
                        style={{
                          width: `${(ally.hp / ally.maxHp) * 100}%`,
                          background: ally.color,
                          boxShadow: ally.hp / ally.maxHp > 0.5 ? `0 0 8px ${ally.color}80` : 'none'
                        }}
