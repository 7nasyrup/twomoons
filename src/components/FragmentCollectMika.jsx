...
// ─── ゲート解錠時 ─────────────────────────────────────────────────────────────
const MIKA_GATE = [
  { speaker: 'ミカ', role: 'MIKA', illust: 'Mika_neutral_injured', text: '「コードが揃いました。……先輩、ゲートを開けましょう」' },
];
...
// ─── メインコンポーネント ─────────────────────────────────────────────────────────
export default function FragmentCollectMika({ onComplete, onSave, onLoad, onToggleSkip, onOpenLog, onToggleAuto, skipMode, autoMode, onOpenSettings }) {
      ...
      {!currentMessage && !isGameOver && (
        <div className="absolute top-4 lg:top-6 left-0 right-0 z-30 flex items-start justify-between px-2 pr-12 lg:px-8 pointer-events-none">
          ...
          <div className="flex flex-col gap-1.5 lg:gap-2 items-end">
            ...
            <div id="fcm-chip-counter" className="glass-panel px-4 py-2 lg:px-6 lg:py-3 rounded-full flex items-center gap-2 lg:gap-3">
            ...
            <div id="fcm-file-counter" className="glass-panel px-4 py-2 lg:px-6 lg:py-3 rounded-full flex items-center gap-2 lg:gap-3">
