...
export default function FragmentCollectNagisa({ onComplete, onSave, onLoad, onToggleSkip, onOpenLog, onToggleAuto, skipMode, autoMode }) {
  ...
  return (
    <div className="absolute inset-0 bg-[#030712] z-50 overflow-hidden select-none">
        ...
        <div className="absolute top-4 lg:top-6 left-0 right-0 z-30 flex items-start justify-between px-2 pr-12 lg:px-8 pointer-events-none">
          ...
          <div className="flex flex-col gap-1.5 lg:gap-2 items-end">
            <div className={`glass-panel px-4 py-2 lg:px-6 lg:py-3 rounded-full flex items-center gap-2 lg:gap-3 ${timeLeft <= 30 ? 'border-red-500/50 animate-pulse text-red-500' : 'text-slate-700'}`}>
              ...
              <span className="text-[11px] lg:text-xs font-orbitron tracking-widest font-bold">{Math.floor(timeLeft / 60)}:{(timeLeft % 60).toString().padStart(2, '0')}</span>
            </div>
            <div id="fcn-chip-counter" className="glass-panel px-4 py-2 lg:px-6 lg:py-3 rounded-full flex items-center gap-2 lg:gap-3">
              ...
              <div className="flex items-center gap-1.5 lg:gap-2">{Array.from({ length: totalChips }, (_, i) => (<div key={i} className={`w-2.5 h-2.5 lg:w-3 lg:h-3 rounded-full border transition-all duration-500 ${i < chipCount ? 'bg-cyan-400 border-cyan-300 shadow-[0_0_8px_rgba(103,232,249,0.8)]' : 'bg-transparent border-white/30'}`} />))}</div>
            </div>
            <div id="fcn-file-counter" className="glass-panel px-4 py-2 lg:px-6 lg:py-3 rounded-full flex items-center gap-2 lg:gap-3">
              ...
              <div className="flex items-center gap-1.5 lg:gap-2">{Array.from({ length: 4 }, (_, i) => (<div key={i} className={`w-2.5 h-2.5 lg:w-3 lg:h-3 rounded-full border transition-all duration-500 ${i < fileCount ? 'bg-green-400 border-green-300 shadow-[0_0_8px_rgba(74,222,128,0.8)]' : 'bg-transparent border-white/30'}`} />))}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {!currentMessage && !isGameOver && (
        <>
          {roomIndex > 0 && (<button onClick={() => moveRoom(-1)} disabled={isTransitioning} className="absolute left-6 top-1/2 -translate-y-1/2 z-30 w-14 h-24 flex items-center justify-center glass-panel rounded-2xl hover:bg-white/10 hover:border-white/30 transition-all duration-200 disabled:opacity-30"><ChevronLeft className="w-8 h-8 text-slate-800" /></button>)}
          {roomIndex < ROOMS.length - 1 && (<button onClick={() => moveRoom(1)} disabled={isTransitioning} className="absolute right-12 lg:right-6 top-1/2 -translate-y-1/2 z-30 w-14 h-24 flex items-center justify-center glass-panel rounded-2xl hover:bg-white/10 hover:border-white/30 transition-all duration-200 disabled:opacity-30"><ChevronRight className="w-8 h-8 text-slate-800" /></button>)}
        </>
      )}
