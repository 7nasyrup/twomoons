...
const createEnemies = () => [
  { id: 'enemy1', name: 'キメラα', image: '/character/kimera3.png', hp: 600, maxHp: 600, color: '#ef4444', isStunned: false, isDead: false, flashTimer: 0 },
];
                ...
                <div className="relative w-12 h-12 lg:w-20 lg:h-20 flex-shrink-0 z-20">
                  ...
                  <div className={`absolute inset-0 bg-slate-900 border-[2px] rotate-45 overflow-hidden shadow-[0_0_15px_rgba(239,68,68,0.3)] ${ally.id === 'nagisa' ? 'border-blue-500/80 shadow-[0_0_15px_rgba(59,130,246,0.3)]' : ally.id === 'mika' ? 'border-pink-500/80 shadow-[0_0_15px_rgba(236,72,153,0.3)]' : 'border-emerald-500/80 shadow-[0_0_15px_rgba(16,185,129,0.3)]'}`}>
                    <div className="absolute inset-0 -rotate-45 scale-[1.4] w-full h-full">
                      <img
                        src={ally.cutinImage || ally.image}
                        alt={ally.name}
                        className="absolute w-[300%] max-w-none object-top"
                        style={
                          ally.id === 'nagisa' ? { top: '15%', left: '55%', transform: 'translateX(-48%)' } :
                            ally.id === 'mika' ? { top: '5%', left: '50%', transform: 'translateX(-45%)' } :
                              { top: '10%', left: '50%', transform: 'translateX(-46%)' }
                        }
                        onError={(e) => {
                          e.target.onerror = null;
                      ...
                      <motion.div
                        ...
                        style={{
                          ...
                          background: hpPercent > 50
                            ? (ally.id === 'nagisa' ? 'linear-gradient(90deg, rgba(59,130,246,0.8), #60a5fa)' : ally.id === 'mika' ? 'linear-gradient(90deg, rgba(236,72,153,0.8), #f472b6)' : 'linear-gradient(90deg, rgba(16,185,129,0.8), #34d399)')
                            : hpPercent > 25
                              ? 'linear-gradient(90deg, #ea580c, #fb923c)'
                              : 'linear-gradient(90deg, #7f1d1d, #ef4444)',
                          boxShadow: hpPercent > 50 ? (ally.id === 'nagisa' ? '0 0 10px rgba(59,130,246,0.8)' : ally.id === 'mika' ? '0 0 10px rgba(236,72,153,0.8)' : '0 0 10px rgba(16,185,129,0.8)') : 'none'
                        }}
                        transition={{ duration: 0.3 }}
                      />
                      {/* Segmentation mask */}
