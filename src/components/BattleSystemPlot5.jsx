...
export default function BattleSystemPlot5({ onComplete, playBGM, stopBGM, playSE }) {
  ...
  // Continuous notes while guarding (Temporarily disabled per user request)
  /*
  useEffect(() => {
    let intervalId;
  ...
  */
  ...
  const handlePointerDown = useCallback((allyId) => {
    ...
    if (stateRef.current.turnPhase === 'enemy_windup' && stateRef.current.activeAttacks.length > 0) {
      ...
      const parryableIndex = activeAttacks.findIndex(attack => {
        ...
        // Tighten parry window: closer to the yellow impact circle (from -150ms to +100ms)
        const parryStart = attack.delay + attack.duration - 150;
  ...
  }, [addLog, addSync, triggerSakuraNote, playSE, triggerSakuraSpeech, hasParried]);
  ...
  }, [playSE, triggerSakuraNote, triggerSakuraSpeech]);
  ...
  }, [healCooldown, addLog, triggerSakuraNote, spawnDamageNumber, triggerSakuraSpeech]);
